...
@Component
@Slf4j
public class HealthCheckWebSocketHandler extends TextWebSocketHandler {

    private static final Map<String, Set<WebSocketSession>> boardSessions = new ConcurrentHashMap<>();
    private final ObjectMapper objectMapper;

    public HealthCheckWebSocketHandler(@Autowired(required = false) ObjectMapper objectMapper) {
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        if (boardId != null) {
            ...
            log.info("Health Check WebSocket connected. BoardId: {}, SessionId: {}, Total connected in board: {}", 
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        ...
        if (boardId != null && boardSessions.containsKey(boardId)) {
            ...
            log.info("Health Check WebSocket closed. BoardId: {}, SessionId: {}", boardId, session.getId());
        }
    }

    @Override
    protected void handleTextMessage(WebSocketSession session, TextMessage message) {
        log.info("Received client Health Check WebSocket message from session {}: {}", session.getId(), message.getPayload());
    }
    ...
    public void broadcastEvent(String boardId, String eventType, Object payload) {
        ...
        if (sessions != null && !sessions.isEmpty()) {
            try {
                ...
                log.info("Broadcasting event '{}' to {} sessions on Health Check board {}", eventType, sessions.size(), boardId);
                broadcastToSessions(sessions, textMessage);
            } catch (Exception e) {
                log.error("Failed to serialize or broadcast event '{}' for Health Check board {}", eventType, boardId, e);
            }
        }
    }
